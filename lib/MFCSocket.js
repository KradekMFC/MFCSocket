...
nconf.defaults({
    // protocol defines the WebSocket protocol that should be used to connect to MFC
    protocol: {
        name: "rfc6455",
        version: 13
    },
    urls: {
        // used to identify to MFC from where the socket connection is coming
        origin: "http://m.myfreecams.com",
        // where on MFC the list of available chat servers is located
        serverConfigFile: "http://www.myfreecams.com/mfc2/data/serverconfig.js",
        // url format used to connect to an MFC server
        socketFormat: "ws://%s.myfreecams.com:8080/fcsl"
    },
    // keep alive ping every 15 seconds
    pingIntervalInSeconds: 15000, 
    defaultUserName: "guest",
    defaultPassCode: "guest",
    // how many characters are used to specify the length of an incoming MFC message
    msgLengthCharCount: 6,
    // magic version number pulled from MFC source; previously 20071025
    loginVersion: "20080909",
    // socket hello, pulled from MFC source; previously "hello fcserver\n\0"
    socketHello: "hello fcserver\n\x00"
});
...
function MFCSocket(name, passCode) {
    ...
    function onSocketOpened(){
        socket.send(SOCKET_CONFIG.socketHello);
        //login as a guest
        var loginString = (name || SOCKET_CONFIG.defaultUserName) + ":" + (passCode || SOCKET_CONFIG.defaultPassCode);
        socket.send(new MFCMessage({Type:MFCMessageType.FCTYPE_LOGIN, Arg1: SOCKET_CONFIG.loginVersion, Data:loginString}).asMFCMessage());
    ...
    function sendMessage(msg){
        ...
        if (socket.readyState === WebSocket.OPEN && null !== sessionId){
